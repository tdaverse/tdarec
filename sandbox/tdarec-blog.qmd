...
We (JCB) have consolidated all currently available vectorizations (of which we are aware) and implemented them in a single R library. For ease of comparison, we also use a consistent interface, with a common camelcase naming convention, e.g. `computePersistenceLandscape()`.
Additionally, several new vectorization methods developed by our group are also available within the TDAvec framework.

Here, for example, is how to compute the tropical coordinates proposed by @Kalisnik2019 and a coarse vectorization of our own persistence block transformation [@Chan2022] for the degree-1 persistence diagram above:
...
recipe(~ sample, data = dat) |> 
  ...
  step_pd_degree(sample_pd, hom_degrees = 1:2) |> 
  step_vpd_persistence_block(
    sample_pd_1, sample_pd_2,
