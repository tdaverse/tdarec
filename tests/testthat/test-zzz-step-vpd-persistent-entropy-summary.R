...
test_that("`step_vpd_persistent_entropy_summary()` agrees with raw function", {
...
test_that("`tunable()` returns standard names", {
  ...
  vpd_rec <- dist_rec |> 
    step_vpd_persistent_entropy_summary(dist_pd, keep_original_cols = FALSE)
...
test_that("recipe with empty selection incurs no `prep()` or `bake()` change", {
  ...
  vpd_rec1 <- dist_rec
  vpd_rec2 <- step_vpd_persistent_entropy_summary(
    vpd_rec1,
    xseq = scale_seq,
    hom_degree = 0,
    keep_original_cols = FALSE
...
test_that("tidy method for empty selection works", {
  ...
  vpd_rec <- step_vpd_persistent_entropy_summary(
    dist_rec,
    xseq = scale_seq,
    hom_degree = 0,
    keep_original_cols = FALSE
