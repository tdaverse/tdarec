...
test_that("`step_vpd_complex_polynomial()` agrees with raw function", {
...
test_that("`tunable()` returns standard names", {
  ...
  vpd_rec <- dist_rec |> 
    step_vpd_complex_polynomial(dist_pd, keep_original_cols = FALSE)
  ...
  expect_equal(
    tun$name,
    c("hom_degree", "num_coef", "poly_type")
  )
...
test_that("recipe with empty selection incurs no `prep()` or `bake()` change", {
  ...
  vpd_rec1 <- dist_rec
  vpd_rec2 <- step_vpd_complex_polynomial(
    vpd_rec1,
    hom_degree = 0,
    keep_original_cols = FALSE
  )
...
test_that("tidy method for empty selection works", {
  ...
  vpd_rec <- step_vpd_complex_polynomial(
    dist_rec,
    hom_degree = 0,
    keep_original_cols = FALSE
  )
