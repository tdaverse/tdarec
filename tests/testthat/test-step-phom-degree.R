...
test_that("`step_phom_degree()` accepts base R classes", {
  rec <- recipe(~ ., data = dat) |> 
    step_phom_degree(df, mat)
...
test_that("`step_phom_degree()` accepts {ripserr} 'PHom' class", {
  ...
  rec <- recipe(~ ., data = dat) |> 
    step_phom_degree(roads, topos)
  expect_no_error(bake(prep(rec, training = dat), new_data = dat))
})

test_that("`step_phom_degree()` accepts {TDA} 'diagram' class", {
