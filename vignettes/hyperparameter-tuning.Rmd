...
The persistent homology of a real-valued function on a manifold is determined by the _sublevel set filtration_:
...
The MNIST handwritten digits data set comprises 70,000 $28 \times 28$ pixellated black-and-white images of the numerals 0--9 obtained from forms completed by US Census Bureau field staff and Maryland high school students.
...
1. One limitation of cubical persistent homology is that it is sensitive only to the height difference between cells in the array, not the distances between them. So, on topographic data, a steep peak and a gentle mound of the same height will be encoded as the same feature.
...
RFs are ubiquitous in ML for their combination of flexibility and performance: They can be used almost anywhere and generally produce more accurate models than average.
...
The accuracy is poor but clearly better than chance. The confusion matrix suggests some reasons for the misclassifications:
Most confused are $0$ and $2$ (5 errors), $0$ and $3$ (5), $0$ and $7$ (7), $1$ and $4$ (7), and $6$ and $8$ (5). These pairings are more geometrically than topologically similar; there were fewer confusions of $6$ and $9$ or of either and $0$ or $4$, where we might have expected them.
...
We euxpect the important topological features of the digits number at most 2 per image, so we manually prescribe a narrow range for `num_levels`, though it could also be learned from the training set.
...
The syntax is similar to that of the grid search, but for illustration we specify 6 initial seeds with 12 tuning iterations each, and we limit the total runtime to 5 minutes.
...
Among the most-confused digits are $1$ and $7$ (7), $2$ and $5$ (9), and $3$ and $5$ (12). This is much more in keeping with the topological similarities of the digits, though geometry is also clearly playing a role.
...
Some patterns in the training set hold up, though others don't, and some frequent confusion pairings (e.g. $2$ and $6$) are new.
