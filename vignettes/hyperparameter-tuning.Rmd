...
As a test of whether the vectorized persistence data are intrinsically informative, we will build a linear discriminant analysis classifier rather than a more flexible but less interpretable support vector machine or random forest.
In anticipation of the optimization process, we can also check its dials.

```{r linear discriminant model}
