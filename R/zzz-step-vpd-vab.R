...
#' 
#' @description The function `step_vpd_vab()` creates
...
#' @example inst/examples/zzz-step-vpd-vab.R

#' @export
step_vpd_vab <- function(
    recipe,
    ...,
    role = "predictor",
    trained = FALSE,
  ...
  recipes_pkg_check(required_pkgs.step_vpd_vab())
...
#' @export
prep.step_vpd_vab <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_vpd_vab <- function(object, new_data, ...) {
  ...
  for (col_name in col_names) {
    ...
    vph_data[[paste(col_name, "vab", sep = "_")]] <- col_vpd
  }
  vph_data <- tidyr::unnest(
    vph_data,
    cols = tidyr::all_of(paste(col_names, "vab", sep = "_")),
    names_sep = "_"
  )
...
#' @export
required_pkgs.step_vpd_vab <- function(x, ...) {
  c("TDAvec", "tdarec")
}

#' @rdname tidy_tdavec
#' @export
tidy.step_vpd_vab <- function(x, ...) {
  if (is_trained(x)) {
    res <- tibble(
...
#' @export
tunable.step_vpd_vab <- function(x, ...) {
  tibble::tibble(
    name = c("hom_degree"),
    call_info = list(
