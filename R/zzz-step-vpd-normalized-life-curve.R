...
#' 
#' @description The function `step_vpd_normalized_life_curve()` creates
...
#' @example inst/examples/zzz-ex-step-vpd-normalized-life-curve.R

#' @export
step_vpd_normalized_life_curve <- function(
    recipe,
    ...,
    role = "predictor",
    trained = FALSE,
  ...
  recipes_pkg_check(required_pkgs.step_vpd_normalized_life_curve())
  
  add_step(
    recipe,
    step_vpd_normalized_life_curve_new(
      terms = rlang::enquos(...),
      trained = trained,
      role = role,
...
step_vpd_normalized_life_curve_new <- function(
    terms,
    role, trained,
    hom_degree,
  ...
  step(
    subclass = "vpd_normalized_life_curve",
    terms = terms,
    role = role,
    trained = trained,
    hom_degree = hom_degree,
...
#' @export
prep.step_vpd_normalized_life_curve <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_vpd_normalized_life_curve <- function(object, new_data, ...) {
  ...
  for (col_name in col_names) {
    col_vpd <- purrr::map(
      new_data[[col_name]],
      \(d) as.vector(TDAvec::computeNormalizedLife(
        as.matrix(d),
    ...
    vph_data[[paste(col_name, "normalized_life_curve", sep = "_")]] <- col_vpd
  }
  vph_data <- tidyr::unnest(
    vph_data,
    cols = tidyr::all_of(paste(col_names, "normalized_life_curve", sep = "_")),
    names_sep = "_"
  )
...
#' @export
print.step_vpd_normalized_life_curve <- function(
    x, width = max(20, options()$width - 35), ...
) {
  title <- "normalized life curve of "
...
#' @export
required_pkgs.step_vpd_normalized_life_curve <- function(x, ...) {
  c("TDAvec", "tdarec")
}

#' @rdname step_vpd_normalized_life_curve
#' @usage NULL
#' @export
tidy.step_vpd_normalized_life_curve <- function(x, ...) {
  if (is_trained(x)) {
    res <- tibble::tibble(
...
#' @export
tunable.step_vpd_normalized_life_curve <- function(x, ...) {
  tibble::tibble(
