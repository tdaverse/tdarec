...
#' @title Persistence Surface Vectorization of Persistent Homology
#' 
#' @description The function `step_vpd_pi()` creates
...
#' @example inst/examples/zzz-step-vpd-pi.R

#' @export
step_vpd_pi <- function(
    recipe,
    ...,
    role = "predictor",
    trained = FALSE,
  ...
  recipes_pkg_check(required_pkgs.step_vpd_pi())
...
#' @export
prep.step_vpd_pi <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_vpd_pi <- function(object, new_data, ...) {
...
#' @export
print.step_vpd_pi <- function(
    x, width = max(20, options()$width - 35), ...
) {
  title <- "Persistence surface of "
...
#' @export
required_pkgs.step_vpd_pi <- function(x, ...) {
  c("TDAvec", "tdarec")
}
