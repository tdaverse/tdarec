...
#' 
#' @description The function `step_vpd_nl()` creates
...
#' @example inst/examples/zzz-step-vpd-nl.R

#' @export
step_vpd_nl <- function(
    recipe,
    ...,
    role = "predictor",
    trained = FALSE,
  ...
  recipes_pkg_check(required_pkgs.step_vpd_nl())
...
#' @export
prep.step_vpd_nl <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_vpd_nl <- function(object, new_data, ...) {
...
#' @export
required_pkgs.step_vpd_nl <- function(x, ...) {
  c("TDAvec", "tdarec")
}
...
#' @export
tunable.step_vpd_nl <- function(x, ...) {
  tibble::tibble(
    name = c("hom_degree"),
    call_info = list(
